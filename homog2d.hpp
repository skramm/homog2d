...
public:
/// Split rectangle by a line, returns set of CPolyline
	template<typename FPT2>
	std::vector<CPolyline_<FPT>>
	splitC( const Line2d_<FPT2>& li ) const
	{
		CPolyline_<FPT> pol(*this);
		return pol.splitC( li );
	}

/// Split rectangle by a line, returns set of OPolyline
	template<typename FPT2>
	std::vector<OPolyline_<FPT>>
	splitO( const Line2d_<FPT2>& li ) const
	{
		CPolyline_<FPT> pol(*this);
		return pol.splitO( li );
	}
...
/// Split Polyline by line, return vector of CPolyline
	...
	splitC( const Line2d_<FPT2>& li ) const
	{
		return p_split( li, typ::IsClosed() );
	}
...
private:
	...
	auto
	p_split( const Line2d_<FPT2>&, const PT& ) const;
...
//------------------------------------------------------------------
/// Private struct, need for \c p_split()
template<typename FPT>
struct PolySplitData
{
	...
	PolySplitData(
		...
		: _nbSegs(s), _inputClosed(ic), _outputClosed(oc), _line(li), _inputPts(inpts)
	{
		_vpts.reserve( s );
	}

	void processCurrent( const OSegment_<HOMOG2D_INUMTYPE>& );
	...
	auto handleIntersection( const Point2d_<FP>& itpt );
...
//------------------------------------------------------------------
/// Process remaining points, called once we have parsed all the segments
template<typename FPT>
auto
PolySplitData<FPT>::processRemaining()
{
	...
	if( _case != 'C' )
	{
		if( _inputClosed )
		{
			for( size_t idx=0; idx<_idxFirstInt; idx++ )
			{
...
//------------------------------------------------------------------
/// Adds the current point (its not about intersections here!)
/**
For input closed polyline, we DO NOT register points until we have found an intersection.
this is because the points lying before the first intersection well be handled once we
processed all the segments (see processRemaining() )
*/
template<typename FPT>
void
...
/**
Return value is a pair, first is a bool. If true, means we can push the output set of point (as second)
as new polyline in the output set
*/
template<typename FPT>
template<typename FP>
auto
		...
		else
		{
			...
			else  // second segment is NOT colinear with the line
			{
				...
				else  // points are on different sides, relatively to the line
				{
					...
					if( _found )                // and we create a new polyline,
					{                           // but only if it is *not* the first intersection
						out = _vpts;
						_vpts.clear();
						_vpts.push_back( _intPt );
						newPoly = true;
...
auto
PolylineBase<PTI,FPT>::p_split(
	const Line2d_<FPT2>& li,  ///< the crossing line
	const PTO&                ///< dummy argument, just there so we can fetch the requested type of output polylines
) const
{
	p_normalizePoly();
	...
	do  // search intersection points
	{
		...
		if( inters1() )
		{
			...
			auto hi = psdata.handleIntersection( int_pt );
	...
	while( psdata.keepOn() ); //current<nbSegs() );
...
//	Line2d_<double> line_ortho;
	Line2d_<double> l3_A, l3_B;


	Segment_<double> s0;
	Segment_<double> sL,sR;
	Segment_<double> s_Top;
...
/// Used in buildSquare()
/**
Returns an array of 4 indexes related to the input container, that holds 4 points.
The first 2 indexes are the ones that are the indexes of the points that are the farthest apart.
The other two are the remaining ones.

For example:
- if the two farthest apart points have the indexes 0 and 2,
then the output array will hold: 0213
- if the two farthest apart points have the indexes 1 and 2,
then the output array will hold: 1203

It computes the 6 possible distances.

\todo 20260328: maybe store the \c vindexs statically? Here, it gets rebuild every time the \c buildSquare() function is called.
*/
template<typename CONT>
auto
...
{
	size_t i=0;
	std::array<std::array<uint8_t,4>,6> vindexs;
	vindexs[i++] = {{0,1,2,3}};
	vindexs[i++] = {{0,2,1,3}};
	vindexs[i++] = {{0,3,1,2}};
	vindexs[i++] = {{1,2,0,3}};
...
// compute all the 6 distances between first two elements of array
	std::array<HOMOG2D_INUMTYPE,6> dists;
	for( auto j=0; j<6; j++ )
		dists[j] = h2d::dist( pts[vindexs[j][0]], pts[vindexs[j][1]] );

// find iterator on max distance
...
//	priv::printVector( dists );
	...
	auto out_idx = std::distance( std::begin(dists), maxd );
	return vindexs[out_idx];
}
...
lying on line \f$l_0\f$
- determine wich of the two remaining point if the farthest away from \f$S_0\f$, call it C.
- build the line \f$l_1\f$ that is perpendicular to \f$S_0\f$ and passes through point C
...
- build the line \f$l_2\f$ from F and fourth point D
- build the two lines \f$l_{3a}\f$ and \f$l_{3b}\f$ that are perpendicular to \f$l_2\f$ and pass
...
template<typename CONT>
auto
buildSquare( const CONT& pts, TMP_Params par=TMP_Params() )
{
	TMP_DebugSquare dbg;
	...
	Segment_<HOMOG2D_INUMTYPE> s0( ptA, ptB );
dbg.s0 = s0;
	auto dist = s0.length();
	auto l0   = s0.getLine();
dbg.dist = dist;

// among the 2 remaining points, find the one that is the farthest away from line s0
	auto ptC = pts[ idxarr[2] ];
	auto ptD = pts[ idxarr[3] ];

	auto dtC = l0.distTo( ptC );
	auto dtD = l0.distTo( ptD );

	if( dtD>dtC )
		std::swap( ptC, ptD );
// at this point, ptC is the farthest away from l0,
// ptD is the closest

	auto pt2 = ptC;
	auto pt3 = ptD;
	if( par.swapFC1 ) //swap Farthest/Closest between C and D
		std::swap( pt2, pt3 );

// compute orthogonal line at that point, and get the two points on that line
// that are at a distance dist
	auto l1 = l0.getOrthogLine( pt2 );
dbg.l1 = l1;

	auto ppts = l1.getPoints( pt2, dist );
dbg.ppts = ppts;
...
dbg.l2 = l2;
	...
	auto l3_A = l2.getOrthogLine( ptA );
	auto l3_B = l2.getOrthogLine( ptB );
dbg.l3_A = l3_A;
dbg.l3_B = l3_B;


	auto pt_resL = l3_A * l2;
	auto pt_resR = l3_B * l2;
dbg.pt_resL = pt_resL;
dbg.pt_resR = pt_resR;
	auto s_Top = l3_A.getOrthogSegment( ptC ); // ou ptD?
	auto li_Top = s_Top.getLine();
dbg.s_Top = s_Top;
	auto pt_res3 = l3_B * li_Top;
	auto pt_res4 = l3_B * li_Top;

using FPTYPE = typename CONT::value_type::FType;
	std::vector<Point2d_<FPTYPE>> vout;

...
```C++
Line2d li( ..., ... ); // some line
auto ppts = li.getPoints( GC_X, coord, d );
Point2d p1 = ppts.first;
Point2d p2 = ppts.second;
```

Upon return, the "first" point will hold the one with smallest 'x' coordinate, or the smallest 'y' coordinate, if x coordinates are equal.
...
[(1)](#paedfapol)
<a name="fn1"></a> The rationale behind this interface is that if we had chosen to pass a point as argument, no guarantee would have been given that the point is effectively lying on the line.
With this function signature, the function will compute the point itself, which removes the need for some checking and error handling.

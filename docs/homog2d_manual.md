...
For type `Segment`, the points are stored internally with the "smallest" one as first
(using x coordinate, or, if equal, using y coordinate).
...
For angles, if one of the type is a line, or an unoriented segment, then the angle will stay in the range [0:+M_PI].
...
For these, you can get the side of point relatively to the segment with `getPointSide()`.
It will return a value of enum type `PointSide`, either `PointSide::Left`, `PointSide::Right` or `PointSide::Neither` if the point is on the supporting line.
And you can get that as a human-readable string with the associated `getString()`:
```C++
OSegment s( Point2d(0,10), Point2d(100,10) );
std::cout << getString(getPointSide(pt)); // print "Right"
```
...
#### 3.4.13 - Simplifying polyline objects
...
#### 3.4.14 - Importing from boost::geometry

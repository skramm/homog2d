...
int main( int, const char** )
{
	...
	for( int i=0; i<nbim; i++ )
	{
		...
		auto x1 = std::cos(angle_r)*kx;
		auto y1 = std::sin(angle_r)*ky;
		Point2d pt( x1,y1);
		auto pt_d = Hdraw * pt;
		...
		seg2_d.draw( im, img::DrawParams().setColor(250,0,0) );
		...
		auto colA = img::DrawParams().setColor(250,250,0);
		auto colB = img::DrawParams().setColor(0,250,250);
		switch( segDistCase )
		{
			case -1:
